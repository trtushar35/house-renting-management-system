...
const props = defineProps(["tenant", "id"]);
...
const form = useForm({
    ...
    password: props.tenant?.password ?? '',
    _method: props.tenant?.id ? "put" : "post",
});

const submit = () => {
    const routeName = props.id
        ? route("backend.tenant.update", props.id)
        : route("backend.tenant.store");
    form
        .transform((data) => ({
            ...data,
            remember: "",
